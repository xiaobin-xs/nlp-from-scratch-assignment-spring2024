...
# Adapted from: https://github.com/facebookresearch/atlas/blob/main/src/evaluation.py
...
def f1(prediction, ground_truth, normalize_fn, return_precision_recall=False):
...
def f1_score(prediction, ground_truths, normalize_fn: Callable[[str], str] = lambda x: x, return_precision_recall=False):
    if return_precision_recall:
        return [max(score) for score in zip(*[f1(prediction, gt, normalize_fn, return_precision_recall=True) for gt in ground_truths])]
